...
- **Background Receives (Nostr):** The application supports receiving Lightning payments in the background via Nostr (`npub.cash`). To handle app restarts and prevent concurrent background processes from creating race conditions (e.g., fetching and swapping the same pending token twice), the frontend utilizes a strict singleton claim loop. Meanwhile, the backend `receive_ecash` command enforces idempotency by detecting "proofs already spent" errors and recovering the output proofs deterministically via a `restore` fallback instead of abandoning the operation.
...
## 7. Hybrid Transaction History Engine

Because the Rust backend natively tracks all fiat swaps as generic Lightning `Mint` or `Melt` operations, the frontend utilizes a **Hybrid History Engine** to provide accurate user-facing labels.

1. **Local State (`momoHistory`):** Whenever a MoMo swap is initiated, the frontend stores the phase, direction, and metadata in `zustand` local storage.
...
## 8. NUT-11 Pay-to-Public-Key (P2PK)
...
- **`src/services/`**: Integration logic for external APIs. Contains `momoService.ts` for Blink fiat proxy interactions and `lnurlService.ts` for LNURL-pay resolution.
- **`src/constants.local.ts`**: Centralized configuration file holding all API endpoints (Blink Proxy) and environment variables.
