  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-background/80 backdrop-blur-sm transition-opacity" onClick={onClose}>
      <div 
        ...
        className="bg-surface-container-high/95 backdrop-blur-xl rounded-t-[2rem] sm:rounded-3xl w-full max-w-[480px] border-t border-x sm:border border-outline-variant shadow-[0_-4px_32px_rgba(0,0,0,0.4)] overflow-hidden flex flex-col relative animate-slide-up sm:animate-fade-in pb-8 sm:pb-4"
      >
        ...
        <div className="flex justify-between items-center px-6 py-4 relative z-10">
          ...
          <button 
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full bg-surface-bright text-on-surface-variant hover:text-on-surface hover:bg-surface-bright/80 transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="p-4 flex flex-col gap-2 relative z-10">
          {options.map((opt, i) => (
            <button
              ...
              className="flex items-center gap-4 p-4 hover:bg-surface-bright/50 transition-colors rounded-2xl border border-transparent hover:border-outline-variant text-left w-full group"
            >
